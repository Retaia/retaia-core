...
final class HeartbeatJobHandlerTest extends TestCase
{
    ...
    {
        $job = new Job('job-1', 'asset-1', 'extract_facts', JobStatus::CLAIMED, 'agent-1', 'token', new \DateTimeImmutable('+5 minutes'), [], [], null, 2);

        $gateway = $this->createMock(JobGateway::class);
        $gateway->expects(self::once())->method('heartbeat')->with('job-1', 'agent-1', 'token', 1, 300)->willReturn($job);
        $gateway->expects(self::never())->method('find');

        $result = (new HeartbeatJobHandler($gateway, new ResolveJobLockConflictCodeHandler($gateway)))->handle('job-1', 'agent-1', 'token', 1);
    ...
    {
        $gateway = $this->createMock(JobGateway::class);
        $gateway->expects(self::exactly(2))->method('heartbeat')->with('job-1', 'agent-1', 'wrong-token', 1, 300)->willReturn(null);
        $gateway->expects(self::exactly(2))->method('find')->with('job-1')->willReturnOnConsecutiveCalls(
            new Job('job-1', 'asset-1', 'extract_facts', JobStatus::CLAIMED, 'agent-1', 'right-token', null, [], [], null, 1),
            null
        );

        $handler = new HeartbeatJobHandler($gateway, new ResolveJobLockConflictCodeHandler($gateway));

        self::assertSame(HeartbeatJobResult::STATUS_STALE_LOCK_TOKEN, $handler->handle('job-1', 'agent-1', 'wrong-token', 1)->status());
        self::assertSame(HeartbeatJobResult::STATUS_LOCK_INVALID, $handler->handle('job-1', 'agent-1', 'wrong-token', 1)->status());
    }
}
